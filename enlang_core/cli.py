...
def run_file(file_path: str, custom_port: int = None):
    ...
    if ext == ".enlgdb":
        ...
        try:
            ...
            dbname = os.path.basename(db_file)
            print(f"\n[ERROR] {fname}: {e}")
            print(f"\n  Cause   : Duplicate value violates PRIMARY KEY or UNIQUE constraint.")
            print(f"  Fix 1   : Delete '{dbname}' and re-run to start fresh.")
            print(f"  Fix 2   : Use different ID values in your insert statements.")
            print(f"  Fix 3   : Use 'update <table> set <col>=<val> where id is <x>' to modify existing rows.\n")
            sys.exit(1)
        except sqlite3.OperationalError as e:
            fname = os.path.basename(file_path)
            print(f"\n[ERROR] {fname}: {e}")
            print(f"\n  Cause   : Invalid table, column, or SQL syntax.")
            print(f"  Fix     : Run 'enlang check {fname}' to inspect your schema.\n")
            sys.exit(1)
